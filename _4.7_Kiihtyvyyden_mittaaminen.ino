...
void loop() {
  ...
  magicWand();           //Komento magicWand ei ole käytössä opiskelijaesimerkissä!
}

void magicWand(){
    int pitch = (xAcc + yAcc + zAcc) * 4; //Harjoituksessa ohjataan muokkaamaan tätä kohtaa koodista siten, että piezosta saadut äänet olisivat mahdollisimman mielenkiintoisia.
                                          // Eräs vaihtoehto jännän äänimaiseman virittämiseen voi olla erilliset if-lauseet x-, y- ja z-suuntaan. Kun kiihtyvyysarvo tiettyyn suuntaan ylittyy, päästää piezo aina uudenlaisen äänen!
    tone(6, pitch);
 }
} 
